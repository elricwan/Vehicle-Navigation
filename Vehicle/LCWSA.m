...
window_size = [600];
batch_size_set =[28,25,22,19,16,13];
...
Niter = 20;
for wstep=1:length(batch_size_set)
w = 0.6;
batch_size = batch_size_set(wstep); 
for ii=1:Niter
A = model.A;
A(1,82) = 0;
...
while sum(xop_update(Dest,Dest_seg,:)) == 0
    ...
    % objective function
    obj = w*sum(sum(sum(ff.*xx(:,:,1:batch_size))))+(1-w)*sum(sum(g(:,:,batch_size).*xx(:,:,batch_size)))*batch_size ...
        -0.07*sum(xx(Dest,Dest_seg,1:batch_size));
    ...
    for road_i=1:N
        for j=1:1:Segment{road_i}.num_sub
            if emptyVRoad(road_i,j) > 0 % we only manage the road that within the range
                Constraint = [Constraint, sum(xx(road_i,j,2:end))+sum(xop_update(road_i,j,:))<=2];
            end
        end
    end
    ...
    i=min(batch_size,length(street));
